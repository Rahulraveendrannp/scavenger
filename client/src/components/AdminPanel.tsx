...
const AdminPanel: React.FC<AdminPanelProps> = () => {
  ...
  useEffect(() => {
    ...
    const sortedUsers = filtered.sort((a, b) => {
      ...
      const claimedCountA = claimsA ? Object.values(claimsA).filter(Boolean).length : 0;
      const claimedCountB = claimsB ? Object.values(claimsB).filter(Boolean).length : 0;
  ...
  const togglePrizeClaim = async (userId: string, prizeType: 'cardGame' | 'puzzle' | 'carRace' | 'scavengerHunt') => {
    try {
      ...
      const response = await ScavengerAPI.updatePrizeClaim(userId, prizeType, newStatus);
      ...
      if (response.success) {
        ...
        setShowSuccessMessage(`Prize claim ${newStatus ? 'marked' : 'unmarked'} for ${user.phoneNumber}`);
        setTimeout(() => setShowSuccessMessage(''), 2000);
        
        console.log('✅ Admin: Updated prize claim:', { userId, prizeType, newStatus });
      } else {
        setError('Failed to update prize claim: ' + (response.error || 'Unknown error'));
        setTimeout(() => setError(''), 3000);
      }
    } catch (err: any) {
      console.error('❌ Admin: Error updating prize claim:', err);
      setError('Error updating prize claim: ' + (err.message || 'Network error'));
      setTimeout(() => setError(''), 3000);
    }
  };
          ...
          <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="text-center sm:text-left">
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-orange-600">Admin Panel</h1>
                <p className="text-sm sm:text-base text-gray-600">Manage prize claims</p>
              </div>
            </div>
          ...
          {filteredUsers.length === 0 ? (
            <div className="p-6 sm:p-8 text-center">
              ...
              <p className="text-sm sm:text-base text-gray-400">
                {searchQuery ? 'Try a different phone number' : 'Users will appear here once they register'}
              </p>
            </div>
          ) : (
            <>
                    ...
                    return (
                      <tr key={user._id} className="hover:bg-orange-50">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            onClick={() => togglePrizeClaim(user._id, 'cardGame')}
                            ...
                            title={claims.cardGame ? 'Prize claimed - click to unmark' : 'Prize not claimed - click to mark as claimed'}
                          >
                            <span
                              className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${
                                claims.cardGame ? 'translate-x-9' : 'translate-x-1'
                              }`}
                            />
                          </button>
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            ...
                            title={claims.puzzle ? 'Prize claimed - click to unmark' : 'Prize not claimed - click to mark as claimed'}
                          >
                            <span
                              className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${
                                claims.puzzle ? 'translate-x-9' : 'translate-x-1'
                              }`}
                            />
                          </button>
                          <div className="text-xs mt-1">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            onClick={() => togglePrizeClaim(user._id, 'carRace')}
                            className={`relative inline-flex h-8 w-16 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 ${
                              claims.carRace 
                                ? 'bg-green-600' 
                                : 'bg-gray-200 hover:bg-gray-300'
                            }`}
                            title={claims.carRace ? 'Prize claimed - click to unmark' : 'Prize not claimed - click to mark as claimed'}
                          >
                            <span
                              className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${
                                claims.carRace ? 'translate-x-9' : 'translate-x-1'
                              }`}
                            />
                          </button>
                          <div className="text-xs mt-1">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            onClick={() => togglePrizeClaim(user._id, 'scavengerHunt')}
                            className={`relative inline-flex h-8 w-16 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 ${
                              claims.scavengerHunt 
                                ? 'bg-green-600' 
                                : 'bg-gray-200 hover:bg-gray-300'
                            }`}
                            title={claims.scavengerHunt ? 'Prize claimed - click to unmark' : 'Prize not claimed - click to mark as claimed'}
                          >
                            <span
                              className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${
                                claims.scavengerHunt ? 'translate-x-9' : 'translate-x-1'
                              }`}
                            />
                          </button>
                          <div className="text-xs mt-1">
            ...
            <div className="lg:hidden">
              {filteredUsers.map((user) => {
                ...
                return (
                  <div key={user._id} className="border-b border-gray-200 last:border-b-0 p-3">
                    {/* User Info */}
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      ...
                      <div className="bg-gray-50 rounded-lg p-2 text-center">
                        <div className="text-xs font-medium text-gray-600 mb-1">Card</div>
                        <button
                          onClick={() => togglePrizeClaim(user._id, 'cardGame')}
                        ...
                        >
                          <span
                            className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${
                              claims.cardGame ? 'translate-x-6' : 'translate-x-1'
                            }`}
                          />
                        </button>
                      </div>
                      ...
                      {/* Puzzle */}
                      <div className="bg-gray-50 rounded-lg p-2 text-center">
                        <div className="text-xs font-medium text-gray-600 mb-1">Puzzle</div>
                        <button
                          onClick={() => togglePrizeClaim(user._id, 'puzzle')}
                          className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors focus:outline-none focus:ring-1 focus:ring-orange-500 ${
                            claims.puzzle 
                        ...
                        >
                          <span
                            className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${
                              claims.puzzle ? 'translate-x-6' : 'translate-x-1'
                            }`}
                          />
                        </button>
                      </div>
                      ...
                      {/* Car Race */}
                      <div className="bg-gray-50 rounded-lg p-2 text-center">
                        <div className="text-xs font-medium text-gray-600 mb-1">Car</div>
                        <button
                          onClick={() => togglePrizeClaim(user._id, 'carRace')}
                          className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors focus:outline-none focus:ring-1 focus:ring-orange-500 ${
                            claims.carRace 
                        ...
                        >
                          <span
                            className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${
                              claims.carRace ? 'translate-x-6' : 'translate-x-1'
                            }`}
                          />
                        </button>
                      </div>
                      ...
                      {/* Scavenger Hunt */}
                      <div className="bg-gray-50 rounded-lg p-2 text-center">
                        <div className="text-xs font-medium text-gray-600 mb-1">Hunt</div>
                        <button
                          onClick={() => togglePrizeClaim(user._id, 'scavengerHunt')}
                          className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors focus:outline-none focus:ring-1 focus:ring-orange-500 ${
                            claims.scavengerHunt 
                        ...
                        >
                          <span
                            className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${
                              claims.scavengerHunt ? 'translate-x-6' : 'translate-x-1'
                            }`}
                          />
                        </button>
                      </div>
